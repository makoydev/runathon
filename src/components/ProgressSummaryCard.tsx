...
export function ProgressSummaryCard({ summary, unit, onResetProgress }: ProgressSummaryCardProps) {
  ...
  return (
    <section
      ...
      className="bg-white/70 backdrop-blur-sm rounded-xl border border-slate-200 p-6 shadow-sm print:hidden"
    >
      <div className="flex items-center justify-between">
        <h3 className="text-xl font-semibold text-slate-700">Progress</h3>
        {markedCount > 0 && (
          <button
      ...
      {markedCount === 0 ? (
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
            <div>
              <div className="text-sm text-slate-500">Workouts Completed</div>
            ...
            <div>
              ...
              <div className="text-xl font-bold text-slate-700">
                {completionPercent}%
                <span className="ml-1 text-sm font-normal text-slate-400">of marked</span>
              </div>
            </div>
            <div>
              <div className="text-sm text-slate-500">Mileage Completed</div>
              <div className="text-xl font-bold text-slate-700">
                {formatDistanceInUnit(summary.completedMileageKm, unit)}
              </div>
            </div>
            <div>
              <div className="text-sm text-slate-500">Longest Run Done</div>
              <div className="text-xl font-bold text-slate-700">
