...
export function PlanComparisonCard({ options, selectedDays, unit, onSelect }: PlanComparisonCardProps) {
  ...
  return (
    ...
    >
      <h3 className="text-xl font-semibold text-slate-700">Compare Schedules</h3>
      ...
      <div className="mt-4 overflow-x-auto">
        <table className="w-full text-sm min-w-[480px]">
          <thead>
            <tr>
              <th scope="col" className="text-left font-medium text-slate-500 p-2" />
              {options.map((option) => {
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.label} className="border-t border-slate-100">
                <th scope="row" className="text-left font-medium text-slate-500 p-2 whitespace-nowrap">
                  {row.label}
                </th>
                {options.map((option) => (
                  <td
